...
4. [ ] **Strict Meta Astryx UI, Custom Tooltips & Viewport-Safe Containment**: Frontend UI code MUST strictly use modern Meta Astryx design tokens and components (`@forge/ui`). ZERO bespoke unapproved CSS or random component libraries. All scrollbars, popups/drawers, modals, notifications/toasts, dropdowns, and tooltips MUST strictly use Meta Astryx custom styling — ZERO OS/browser defaults (including browser default `title` tooltips). All dropdown menus, popovers, tooltips, and dialogs MUST implement smart collision detection (auto-flip/shift/clamp) to guarantee 100% visibility inside the active viewport window without clipping or overflowing.
...
- **MANDATORY**: All UI views, cards, modals, tables, headers, buttons, dropdowns, popups, tooltips, and notifications across the platform MUST strictly use Meta Astryx design system tokens and component wrappers (`@forge/ui`).
- **ZERO Browser Defaults**: OS-default scrollbars, native `alert()`/`confirm()`/`prompt()` dialogs, unstyled `<select>` dropdowns, default popups, and raw OS `title` tooltips are **STRICTLY FORBIDDEN**. Scrollbars must be slim, themed Astryx tracks. Notifications must use Astryx Toast overlays. Dropdowns must use styled custom selectors. Popups/modals must use Astryx glassmorphic backdrop dialogs. Tooltips and informational hover popovers must strictly use Meta Astryx custom floating tooltips (`data-astryx-tooltip` / `getAstryxTooltipScript()`).

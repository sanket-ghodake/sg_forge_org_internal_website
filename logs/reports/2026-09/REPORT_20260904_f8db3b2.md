...
- **Commit**: `f8db3b2` (`f8db3b29866724f2531304cdabc5e5ea23b72bc9`)
- **Timestamp**: `2026-09-04 16:45` (`2026-09-04T16:45:51+05:30`)
- **Author**: `Sanket Ghodake`
- **Conventional Type**: `refactor` | **Scope**: `config`
- **Subject**: `standardize .env and .env.example with zero-trust security and integrity tests`
...
```
refactor(config): standardize .env and .env.example with zero-trust security and integrity tests

- Prune dead PROJECT_NAME variable from .env and .env.example
- Document air-gapped ingress TLS/HTTPS options and 6-parameter service registry
- Configure dynamic token expirations (JWT_ACCESS_TOKEN_EXPIRY_SECONDS / JWT_REFRESH_TOKEN_EXPIRY_SECONDS)
- Harden Developer Dashboard production authentication against template default passwords
- Support dynamic session cookie extraction across Auth microservice endpoints
- Synchronize micro-app scaffolding to update both .env and .env.example
- Add Tier 1 env-integrity.test.ts automated parity and port mapping suite
```
...
- **Total Insertions (+)**: `+2367`
- **Total Deletions (-)**: `-1105`
- **Affected Subsystems**: `auth`, `dev-dashboard`, `sdk`, `toolchain`

---

## 🗂️ Detailed File Changes (14 Files)

| Action | File Path |
| :--- | :--- |
| 🟡 Modified | `.env.example` |
| 🟡 Modified | `apps/src/auth/src/backend/api-org-handlers.ts` |
| 🟡 Modified | `apps/src/auth/src/backend/audit-session-controller.ts` |
| 🟡 Modified | `apps/src/auth/src/backend/session-manager.ts` |
| 🟡 Modified | `apps/src/dev-dashboard/src/backend/auth-session.ts` |
| 🟡 Modified | `apps/src/dev-dashboard/test/security/dashboard-auth.test.ts` |
| 🟢 Added | `apps/src/sdk/test/unit/env-integrity.test.ts` |
...
| 🟡 Modified | `scripts/test/create-app.test.ts` |
